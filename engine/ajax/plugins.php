			...
			foreach ($xml->file as $file) {
				...
				if ( $file->operation ) {
					foreach ($file->operation as $operation) {
						...
						if($operation->enabled) $_POST['filedisable'][$i][$t] = (int)trim($operation->enabled);
						if($operation->dleversion) $_POST['filedleversion'][$i][$t] = trim((string)$operation->dleversion);
