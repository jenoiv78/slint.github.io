...
if ($_REQUEST['action'] == "del_social") {
	...
	if( $_REQUEST['user_hash'] == "" OR $_REQUEST['user_hash'] != $dle_login_hash OR !$member_id['user_id'] OR !$id) {
		die ("error");
	}
...
if ($_REQUEST['action'] == "viewshared") {
	...
	if( $_REQUEST['user_hash'] == "" OR $_REQUEST['user_hash'] != $dle_login_hash OR !$member_id['user_id']) {
		die ( "{\"error\":\"{$lang['err_notlogged']}\"}" );
	}
...
HTML;
	...
	foreach ( $files['files'] as $entryname ) {
		...
		if ( $config['file_driver'] ) {
			$http_url = $config['remote_url'].'shared/';
		} else {
			$http_url = $config['http_home_url'] . "uploads/shared/";
		}
	...
	<div class="file-footer">
		...
		<div class="file-footer-bottom">
			<div class="file-preview">
				<a onclick="document.activeElement.blur(); hs.expand(this); return false;" href="{$img_url}" rel="tooltip" title="{$lang['up_im_expand']}"><i class="mediaupload-icon mediaupload-icon-zoom"></i></a>
