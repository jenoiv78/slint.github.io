...
if( ! $allow_addnews ) {
	...
	if( isset( $_REQUEST['mod'] ) AND $_REQUEST['mod'] == "addnews" AND $is_logged AND $user_group[$member_id['user_group']]['allow_adds'] ) {
		...
		if( !isset($_POST['duplicateprotection']) OR !isset($_SESSION['dp_'.$_POST['duplicateprotection']]) OR ( isset($_SESSION['dp_'.$_POST['duplicateprotection']]) AND $_SESSION['dp_'.$_POST['duplicateprotection']] !== true ) ) {
		...
		if( !$stop ) {
			...
			if( $found ) {
				...
				if($approve) {
					...
					$row = $db->super_query( "SELECT id, date, category, alt_name FROM " . PREFIX . "_post WHERE id='{$row['id']}'" );
			...
			unset($_SESSION['dp_'.$_POST['duplicateprotection']]);
		...
		$_SESSION['dp_'.$duplicateprotection] = true;
